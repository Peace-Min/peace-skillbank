...
4. **`scripts/finalize-handoff.py`로 마무리**한다(결정론적):
   - 시크릿(키·토큰·비번·연결문자열·개인키) **자동 마스킹**, 절대경로/이메일/IP는 **검토 플래그**,
...
프롬프트는 **폐쇄망을 나가 외부 모델로 간다.** 정식 검수는 필요 없지만, **붙여넣기 직전에 마스킹
리포트와 본문을 한 번 훑어** 시크릿/사내명이 안 샜는지만 확인하라. 시크릿은 자동 마스킹되지만,
정규식이라 모든 형태를 다 잡진 못한다(그래서 경로/사내명은 플래그만 한다). 외부로 새는 시크릿은
유일하게 되돌릴 수 없는 사고다.

## 검증됨

`qwen2.5-coder:7b`(Ollama, 배포 타깃 27b보다 더 약함)로 E2E 확인: 약한 모델이 6섹션 템플릿을 채우고,
초안에 넣은 DB 비번·API 키를 `finalize-handoff.py`가 마스킹했으며, 응답 지시문이 항상 붙었다.
