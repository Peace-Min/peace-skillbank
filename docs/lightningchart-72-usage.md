...
- 이 스킬은 **별도 command alias 없이 동작**한다. `SKILL.md`의 `name`/`description`으로 plugin이
  자동 발견하므로, 설명에 맞는 LC 7.2 질문에서 트리거되거나 위 namespace 형태로 직접 부른다.
  `commands/lightningchart-72.md`는 필요 없다 — `commands/`의 alias는 diagsession처럼 *짧은
  비-namespace 진입점*(`/lightningchart-72`)이 필요할 때만 추가하는 선택 사항이다(README 참고).
