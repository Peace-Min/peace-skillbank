...
| Python 3 | 검증 스크립트(`verify-symbols.py`, 에이전트가 직접 실행) | 질의응답 시 사용(자동 훅 아님) |
...
- 호출 방식 3가지: (a) LC 7.2 질문을 하면 `SKILL.md`의 `description`으로 **자동 트리거**,
  (b) namespace 형태 `/peace-skillbank:lightningchart-72`로 명시 호출,
  (c) 짧은 `/lightningchart-72` 형태는 `commands/lightningchart-72.md`를 대상 환경의
  `.claude/commands/`에 복사하면 개인 command로 등록된다(diagsession과 동일).
...
Agent를 경계로 접었다. 아래 시퀀스는 발생 가능한 모든 분기와 검증 스크립트(exit 0/1/2)를 포함한다.

> **중요(용어):** 아래 "3) 검증 스크립트" 박스는 **에이전트가 직접 실행**하는 단계이지, 하네스가
> 답변 후 자동으로 돌리는 Claude Code *hook이 아니다*. 즉 다이어그램에 그려져 있어도 **항상 일어난다는
> 보장은 없으며**(에이전트가 건너뛰면 검증되지 않는다), 특히 약한 로컬 모델에서는 안 돌 수 있다.
> 확실히 강제하려면 사용자가 직접 돌리거나 Stop hook을 연결해야 한다(이 레포엔 hook 없음).
...
sequenceDiagram
    ...
    alt 코퍼스 미생성
        ...
        Note over A,S: 3) 검증 스크립트 (계약상 필수 · 에이전트 실행, 자동 hook 아님)
...
- **Tier 1 (DLL 인덱스)** = 존재·시그니처·생성자 arity의 권위. `api-symbols.txt` grep으로 확인하고, 740KB짜리 `api-index.json`은 직접 Read하지 않는다(컨텍스트 폭발 방지; verify 스크립트만 파싱).
...
- **검증 스크립트**(에이전트가 직접 실행, *자동 hook 아님*) `verify-symbols.py --strict`: `exit 0`=전부 검증(단정 가능) / `exit 1`=가짜 심볼·잘못된 `new Type(...)` arity·bare 멤버 → 제거·수정 후 재검증 / `exit 2`=코퍼스 미생성 → 인용 없이 빌드 안내. 강제가 필요하면 직접 돌리거나 Claude Code Stop hook을 연결한다(이 레포엔 hook 없음).
...
- **검증 스크립트는, *실행했을 때*, 존재 + 생성자 arity를 확인**한다(자동 실행되는 harness hook이 아니라
  에이전트가 직접 돌려야 하는 단계 — 안 돌리면 검증되지 않는다). 메서드 매개변수 *타입*까지는 검증하지 않으므로,
