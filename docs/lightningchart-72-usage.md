...
**Skill**은 *절차(SKILL.md) + 스크립트 + 로컬 코퍼스*다. 아래 시퀀스는 발생 가능한 모든 분기와
검증 훅(exit 0/1/2)을 포함한다.
...
sequenceDiagram
    ...
    participant S as Skill (SKILL.md + scripts + 로컬 코퍼스)

    U->>A: LightningChart 7.2 질문
    A->>S: 트리거 -> SKILL.md(계약) 로드
    A->>L: SKILL.md + 질문 주입
    ...
    alt 코퍼스 미생성
        ...
        L-->>A: 인용 없이 "코퍼스 미생성"
        A-->>U: 빌드 안내 (build-api-index / build-manual-index)
    else 코퍼스 있음
