# FORWARD_NULL — null 값 역참조 (forward null dereference)

- **건수**: 47  |  **심각도**: 매우위험  |  **트랙**: C
- **Sparrow 설명**: 어떤 경로에서 `null`일 수 있음이 확인/암시된 참조를, 이후 코드에서 null 검사 없이 역참조(`.멤버`, 인덱싱, 메서드 호출)한다. 앞선 분기(`if (x == null)`, `x = 반환값(가능 null)` 등)에서 null 가능성이 성립된 뒤 그 변수를 그대로 쓰는 흐름을 잡는다.
...
- 무기체계 보안약점 점검 목록: 미매핑 (187 문서 확보 시 항목번호 기입 — "널 포인터 역참조" 계열)
- CERT-C/행안부/OWASP: 행안부 SW보안약점 "널 포인터 역참조"; CERT EXP34-C
