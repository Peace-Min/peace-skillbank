...
- (내부 전용 로그 파일로만 가는 경우는 대개 허용 — 아래 참고.)

## 이렇게 보여도 넘기지 말 것
아래처럼 "노출 아님"으로 보여도 **스킵하지 말고**, 싱크를 확인해 처리한다(전건 수정).
- 대상이 **폐쇄망 내부 운영 로그**로만 흘러가고 외부에 노출되지 않는 것처럼 보여도(방산 폐쇄망이라도) — 그 싱크가 정말 내부 로그인지 확인한다. 내부 로그면 노출 아님을 근거로 남기고(스킵이 아니라 판정 근거 기록), 외부 관측 가능하면 내부 로그 분리 + 일반화 메시지로 수정한다.
- `#if DEBUG` 로 디버그 빌드에서만 상세 출력하는 것처럼 보여도 — 운영 빌드 경로에도 노출이 없는지 확인 후 처리한다.
- 노출값이 실제로는 비민감(고정 문구)한 것처럼 보여도 — 값이 정말 고정/비민감인지 확인해 판단 근거를 남긴다. 근거 없이 건너뛰지 않는다.


## LLM 판단에 필요한 필수 문맥
- catch 블록 내부에서 출력되는 값(`ex.Message`, `ex.ToString()`, `StackTrace`, 경로, 환경정보 등).
- 출력 sink가 UI, API 응답, 표준 출력, 파일 로그, 내부 전용 로그 중 무엇인지.
- 로그 접근 통제와 운영/디버그 빌드 조건(`DEBUG`, trace level 등).
- 사용자에게 보이는 메시지와 내부 로그가 분리되어 있는지.

## 문맥 부족 시 보류 기준
- 출력 호출 한 줄만 있고 sink가 외부 노출인지 내부 로그인지 알 수 없으면 보류한다.
- `ex.Message`가 UI에 표시되는지 내부 진단 로그인지 판단할 수 없으면 `needs_context=true`로 둔다.
- DEBUG 전용 코드인지 운영 코드인지 알 수 없으면 보류한다.

## 추가로 요청해야 할 코드 범위
- 검출 catch 전체와 출력 호출 주변 코드.
- 출력/로그 헬퍼 메서드 구현부.
- UI 바인딩 또는 API 응답으로 이어지는 호출자 경로.
- 빌드 조건과 로깅 설정이 있는 파일.
...
## 기본 처리 분류
- [ ] 수정 → 검출 라인을 위 패턴으로 고침 (내부 로그 분리 + 일반화 메시지)
- [ ] 보류 → 문맥 확보 후 수정 (싱크가 외부인지 판단 불가 시 needs_context; 확보 후 반드시 수정; frontier-handoff)
