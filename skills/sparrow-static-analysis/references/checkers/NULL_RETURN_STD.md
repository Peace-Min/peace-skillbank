...
## 지켜야 할 규칙 (무엇을 왜 검출)
C# 표준 라이브러리(BCL)에는 **실패/부재 시 null 을 반환**하는 메서드가 많다(문서화된 계약). 이런 메서드의 반환값을 null 검사 없이 곧바로 역참조하면 `NullReferenceException`으로 프로세스가 중단된다. 이는 NULL_RETURN 의 **BCL 특수형**(소비측 결함)으로, 반환이 null 일 수 있음이 라이브러리 계약에 이미 명시되어 있으므로 오검출률이 낮고 지적의 근거가 분명하다. 방산 상주/제어 소프트웨어에서 이는 가용성 결함이다.
...
## 이렇게 보여도 넘기지 말 것
아래처럼 "null 불가"로 보여도 **스킵하지 말고**, 근거를 확인해 처리한다(전건 수정).
- 입력이 항상 유효해 해당 BCL 메서드가 실제로는 null 을 반환하지 않는 것처럼 보여도 — null 이 불가능한 근거가 코드에 실제로 있는지 확인한다. 있으면 판정 근거로 남기되, 코드로 드러나지 않으면 반환을 지역변수로 받아 null 검사를 추가해 수정한다.
- 앞선 검증(예: 존재 확인/형식 등록 보장) 이후라 non-null 이 확정되는 것처럼 보여도 — 그 검증이 실제 경로에 있는지 확인하고, 확정이 코드로 드러나지 않으면 가드를 추가한다.
- 근거를 지금 판단할 수 없으면 `문맥 필요`로 표시하고, 문맥 확보 후 반드시 수정한다.
...
- API 계약상 null 반환 가능 여부가 guide/계약표에 없으면 `문맥 필요`로 표시한다.
- 입력값이 항상 유효해 null이 불가능하다는 보장이 코드 조각에 없으면 null 불가라고 단정하지 말고 가드를 추가한다.
...
- [ ] 문맥 필요 → BCL 반환 계약·입력 보장을 확보한 뒤 반드시 수정.
