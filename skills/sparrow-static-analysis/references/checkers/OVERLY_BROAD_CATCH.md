...
- 호출 API가 던지는 예외형이 문서화되지 않아 방어적으로 넓게 잡은 상호운용 경계처럼 보이면 — 예외형 목록(문맥)이 필요하므로 `문맥 필요`로 표시하고, 확보 후 예외형별 명시 catch 로 수정한다. 근거 없이 스킵하지 않는다.
...
- 단순 로깅처럼 보여도 이후 재던지기/격리/상태복구 여부가 누락되면 `문맥 필요`로 표시한다.
