...
## 이렇게 보여도 넘기지 말 것
아래처럼 "이미 안전"으로 보여도 **스킵하지 말고**, 노출 형태를 확인해 처리한다(전건 수정).
- 이미 `ReadOnlyCollection<T>`/`IReadOnlyList<T>`/`IEnumerable<T>` 로 노출해 외부 변경 불가한 것처럼 보이면 — 그 래핑이 실제로 있는지 확인한다. 있으면 판정 근거로 남기고, 원본 컬렉션이 그대로 새는 경로가 있으면 읽기전용 노출/스냅샷으로 수정한다.
- 내부적으로도 불변으로만 쓰이고 공유가 의도된 설계처럼 보여도 — 방산 신뢰성 기준상 읽기전용 노출/방어복사로 캡슐화하는 편이 안전하니 가능하면 수정한다.
- DTO/레코드류 순수 데이터 홀더처럼 보여도 — 캡슐화가 필요 없는 근거를 확인해 판정에 남긴다(스킵이 아니라 근거 기록).
...
## 기본 처리 분류
- [ ] 수정 → 검출 라인을 위 패턴으로 고침 (읽기전용 노출 / 스냅샷 / 관문 메서드)
- [ ] 보류 → 문맥 확보 후 수정 (호출측 파급 판단 필요 시 needs_context; 확보 후 반드시 수정; frontier-handoff)
