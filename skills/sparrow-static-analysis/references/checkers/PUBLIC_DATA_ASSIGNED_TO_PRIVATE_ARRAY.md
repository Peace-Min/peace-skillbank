...
## 이렇게 보여도 넘기지 말 것
아래처럼 "공유가 의도"로 보여도 **스킵하지 말고**, 방어복사가 기본임을 전제로 처리한다(전건 수정).
- 의도적으로 **공유(참조 전달)가 설계**인 것처럼 보여도(대용량 버퍼 성능 최적화, 소유권 이전) — 방산 신뢰성 기준에선 방어복사가 기본 권고다. 성능 핫패스라 복사가 부담이면 그 트레이드오프 근거를 확인해 판정에 남기되, 기본은 방어복사로 수정한다.
- 배열이 사실상 불변으로만 쓰이고 외부·내부 모두 수정 안 하는 것처럼 보여도 — 실제로 수정 경로가 없는지 확인하고, 불확실하면 입력 시 방어복사를 추가한다.
...
## 기본 처리 분류
- [ ] 수정 → 검출 라인을 위 패턴으로 고침 (입력 시 방어적 복사)
- [ ] 보류 → 문맥 확보 후 수정 (성능/소유권 트레이드오프 판단 필요 시 needs_context; 확보 후 반드시 수정; frontier-handoff)
