...
- **소유권 이전처럼 보이는 경우**: 만든 자원을 필드에 저장하거나 호출측에 반환/전달하여 **해제 책임이 이 스코프에 없어 보임**(예: `return reader;`, DI 컨테이너·팩토리 반환). 여기서 Dispose 하면 오히려 버그이므로, 호출자/필드 소유자가 실제로 해제하는지 확인한다. 확인이 필요하면 `문맥 필요`로 표시하고 필요한 문맥에 "호출자/소유자 Dispose 계약"을 적은 뒤, 소유자 측에서 Dispose 패턴을 갖추도록 수정한다. 근거 없이 스킵하지 않는다.
...
- 반환 또는 필드 저장으로 소유권이 이전되는지 알 수 없으면 `문맥 필요`로 표시한다.
