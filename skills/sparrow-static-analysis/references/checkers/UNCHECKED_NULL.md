...
## 이렇게 보여도 넘기지 말 것
아래처럼 "null 아님"으로 보여도 **스킵하지 말고**, 검증 흐름을 확인해 처리한다(전건 수정).
- 직전에 non-null 대입이 있는 것처럼 보여도 — 그 대입이 모든 경로에서 확정되는지 확인하고, 확정이 코드로 드러나지 않으면 가드를 추가한다.
- `as` 뒤 곧바로 `is`/`!= null` 이 아닌 **다른 형태의 검증**(예: 별도 헬퍼 `EnsureNotNull`)을 통과하는 것처럼 보여도 — 그 헬퍼가 실제로 non-null 을 보장하는지 확인한다. 판단 불가하면 `보류`(needs_context)로 두고 확보 후 수정한다.
- 프레임워크가 non-null 을 보증하는 잘 알려진 속성(예: `sender`가 관례상 non-null)처럼 보여도 — 그 관례가 이 호출 경로에 성립하는지 확인해 판정 근거를 남긴다. 근거 없이 건너뛰지 않는다.
...
- [ ] 보류 → 문맥 확보 후 수정 (판단 불가 시 needs_context; 지어내지 않고 문맥 확보 후 반드시 수정; frontier-handoff)
