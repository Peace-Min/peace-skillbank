...
이 문서는 `NULL_RETURN_STD` 처리 시 로컬 Claude가 우선 참조할 정적 계약표다. 목적은 "표준 라이브러리 호출 결과가 null 일 수 있는가"를 빠르게 확인하는 것이며, 여기 없는 API는 임의 단정하지 말고 `문맥 필요`로 표시한다.
...
- 표에 없는 API는 공식 계약이 요청 패키지에 포함되지 않는 한 `문맥 필요`로 표시한다.
