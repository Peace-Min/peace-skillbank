...
> 프로젝트 정책을 강제한다. 이 정책은 기존 가이드의 "이렇게 보여도 넘기지 말 것" 섹션을
> **"이렇게 보여도 넘기지 말고 이렇게 고쳐라"**로 확정한다.

## (공통) 처리 정책

- Policy A: Sparrow가 나온 항목은 **전건(全件) 수정 대상**이다.
- 모든 검출 항목은 수정 대상이며, 어떤 항목도 스킵/제외하지 않는다(전건 수정).
- 실제로 파일을 고쳤으면 `수정 완료`, patch/Before-After만 낸 경우 `패치 제안`으로 분류한다.
- 소스/예외목록 등 문맥이 부족하면 `문맥 필요`로 분류한다. **단, 이는 "안 함"이 아니라
  "문맥 확보 후 반드시 수정"하는 대기 상태다.** 필요한 파일/심볼/범위를 적는다.
- 파일 접근이 가능하면 반드시 열어 전체 흐름을 확인한 뒤 수정한다. **접근이 불가능한 경우 이는 회피 사유가
  아니며**, 스니펫 범위에서 안전하게 적용 가능한 최소 patch/Before-After를 작성하고 상태를 `패치 제안`으로
  한다(이 경우는 blind 수정에 해당하지 않는다).
- 커밋은 **체커 단위**로 묶는다.
...
- **예외 열거 = LLM 직접**: try 본문의 코드·호출에서 **발생 가능한 예외를 LLM이 직접 열거**해 예외형별 명시 catch를 작성한다(파생→기반 순서). 로컬 .NET Framework reference XML의 `<exception>` 문서가 있으면 그 예외 목록을 **교차 근거로 사용(선택)**한다.
- **완전성 주의**: OVERLY_BROAD_CATCH 체커는 `catch(Exception)` 여부만 보므로 명시 catch 일부만 있어도 통과하지만, 신뢰성시험은 **발생 가능 예외의 누락 없는 처리**를 요구한다. 확신할 수 있는 예외형만 명시하고, **확신 없는/미상 예외형은 추측하지 말고 문맥 필요** 로 두고 문맥·예외맵 확보 후 수정한다.
- 빈 catch 금지 → `LogUtil.Error(this, ex.ToString())`로 기록. 각 명시 catch 본문은 LogUtil.Error + **원본 catch 로직(return/rethrow 등) 보존**.
- 예외 열거가 부족하면 skip이 아니다 → 예외목록/소스를 확보한 뒤 **반드시 수정**한다.
