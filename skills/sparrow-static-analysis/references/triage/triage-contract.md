...
> 스크립트는 절대 수정/보류를 스스로 판정하지 않는다. 스크립트는 매칭·조립·집계·게이트만 한다.
...
- 체커 가이드: `references/checkers/<체커키>.md` (Track C 13종). **파일명 == 체커 키(verbatim)**.
  각 가이드 섹션: `## 지켜야 할 규칙` · `## 표준 매핑` · `## 결함 판별 기준` ·
...
```
 [입력]  items/*.md  +  index.csv   (SparrowXlsExport 산출물)
    ...
    │      근거는 **반드시 가이드의 결함판별/`이렇게 보여도 넘기지 말 것` 기준에서** 인용.
    ...
    │      by-checker/<체커>.md = 그 체커의 수정목록(커밋 단위 후보)+보류.
    │      형식 위반 verdict → invalid.csv.
    │
    ▼  ── [사람 + OSTES 코드측] 수정 적용 ─────────────────────────────
    │      수정 항목만 가이드 수정패턴대로 실제 코드 수정(= OSTES 레포측 작업).
...
| 수정/보류 판정, 수정 코드 작성 | LLM 또는 사람 | **판단** |
...
조건부 필드(collect가 강제):
- `verdict = 수정` ⇒ `fix.before` / `fix.after` 채움. `hold_reason`은 빈값.
...
- **G3 — 사람 승인**: 수정·보류(문맥 확보 계획)을 사람이 최종 검토·승인 후 커밋.
...
# 4) [사람/OSTES] 수정 항목만 가이드 수정패턴대로 실제 코드 수정 → 빌드(G1) → 체커단위 커밋
