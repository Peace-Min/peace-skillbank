...
- 실제 처리 대상은 Track A/B 적용 후 잔여 항목이다. GUI Track C는 잔여 엣지 케이스를 놓치지 않도록 Track A/B/C 가이드를 모두 포함한다.
- 체커 가이드는 `references/checkers/<체커키>.md`를 사용한다.
- 전용 체커 가이드가 없어도 항목을 버리지 않는다. XLS의 체커명, 체커 설명, 소스 코드, 파일, 라인을 근거로 fallback 가이드를 생성해 `requests/`를 만든다.
...
- 체커 키 누락 또는 item md 누락처럼 정상 요청 조립이 실패한 행도 버리지 않고 `requests/_UNRESOLVED/`에 별도 md로 남긴다.
...
- `requests/_UNRESOLVED/`: 조립 실패 행. LLM/사람은 원본 XLS와 실제 소스 파일을 확인해 같은 전건 수정 정책으로 처리한다.
...
- 체커 가이드 전문. 전용 가이드가 없으면 XLS 기반 fallback 가이드가 포함된다.
- Sparrow 검출 항목 전문.
- 프로젝트 공통 처리 정책.
- 체커별 프로젝트 의무.

LLM/사람은 요청 md를 읽고 실제 소스 파일 접근이 가능하면 직접 수정한다. 직접 수정할 수 없는 환경이면 다음 형식의 Markdown 수정 지시 또는 patch를 작성한다.

상태 값은 4종이며 요청 md의 출력 형식과 동일해야 한다.

- `수정 완료`: 실제 소스 파일을 고쳤다.
- `패치 제안`: 파일 접근이 불가해 적용할 최소 Before/After 또는 patch만 낸다.
- `문맥 필요`: 지금 수정할 수 없는 이유와 필요한 추가 문맥을 쓴다.
- `판정 근거 기록(수정 불요)`: 결함이 아님을 **코드 인용 + 잔여 리스크 + 소유 계약 출처**로 모두 입증했을 때만 쓴다. 추정만으로는 금지.

false-positive skip은 없다. `문맥 필요`는 스킵이 아니라 문맥 확보 후 수정해야 하는 대기 상태다.
fallback(미등록 체커) 요청도 동일하게 처리한다. 단 근거 범위가 `[검출 항목]`의 `체커 설명` 한 줄과 표시된 소스 라인으로 제한되므로, 요약 대신 **인용**(`근거(인용)` / `근거(코드)`)을 쓴다. 없는 룰·표준 매핑을 추론해 보충하지 않는다. 룰 등록 경로(GUI '체커 룰 관리')는 작업자가 수행할 수 없는 동작이라 요청 md에 넣지 않고, prepare 요약과 해당 체커 폴더의 `_작업지침.md`에만 남긴다.
...
- G2 Sparrow 재분석: 대상 체커 검출 감소/소멸과 신규 검출 여부 확인.
  - 판정은 `Compare-Sparrow.ps1`(count-based)로 한다. 정체성 = (체커 키, **전체경로**)이며 **라인 번호는
    정체성에서 제외**한다 — 수정으로 라인이 밀려도 신규로 오탐하지 않는다. "신규" = (체커,전체경로)별 건수
    증가, "해소" = 건수 감소.
  - **전제(스캔 위생)**: before/after xls 는 반드시 **동일 체크아웃 + 동일 스캔 스코프**의 재분석이어야 한다.
    도구가 두 xls 의 경로 집합을 대조해 불일치 시 경고를 출력하며, `-StrictScope` 지정 시 FAIL 로 승격한다.
  - 파서 exe 는 소스 변경 후 재빌드/재발행된 것을 써야 한다(오래된 publish exe 는 '경로' 컬럼을 모를 수 있음).
