...
```
 [입력]  items/*.md  +  index.csv   (SparrowXlsExport 산출물)
    ...
    │      확신 없으면 지어내지 말고 verdict=보류, needs_frontier=true.
    │      약한 로컬 모델이면 보류/난이도 높은 항목은 frontier-handoff 스킬로 상위 모델 이관
    │      (요청 payload가 이미 self-contained이라 그대로 넘김).
    │
...
- `verdict = 보류` ⇒ `hold_reason` 채움 + 필요시 `needs_frontier=true`. `fix`는 비움.
...
#    (약한 로컬 모델이면 보류 항목을 frontier-handoff 스킬로 상위 모델에 그대로 넘김)
