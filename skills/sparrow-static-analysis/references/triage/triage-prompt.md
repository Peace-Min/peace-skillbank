...
3. 항목에 **`## 수정 대상`** 이 있으면 그 파일/라인/대상 코드를 최우선으로 본다.
   `TARGET LINE` 표시가 있는 경우, `fix.lines`는 그 라인을 포함해야 하며 주변 줄은 필요한 최소 범위만 포함한다.
   표시 없는 주변 코드는 문맥용이며, 별도 근거 없이 임의 수정하지 마라.
4. **확신이 없으면 지어내지 마라.** 소스가 잘린 경우, null/자원 근원이 이 스니펫 밖(외부 경로)이라
...
5. **진성**이면 가이드의 `## 수정 패턴 (C# 예시)`를 따라 **.NET Framework 4.7.2 / C# 7.3** 문법으로
...
6. **보류**면 `hold_reason`에 왜 지금 못 고치는지를 적고, 소스/예외목록 등 문맥이 부족하면
   `needs_context = true` + `missing_context`(필요한 파일/심볼/코드 범위)를 채워라. `fix`는 비운다.
   보류는 스킵이 아니라 **문맥 확보 후 반드시 수정**하는 대기 상태다.
7. `cwe`는 가이드 `## 표준 매핑`의 CWE를 그대로 기입(예: `CWE-476`). `weapon_item`은 아직
   187 매핑 추출 전이므로 `미매핑(187 추출 후 기입)`으로 둔다.
8. **출력은 아래 JSON 스키마 객체 하나만.** 다른 텍스트·마크다운·설명을 앞뒤에 붙이지 마라.
