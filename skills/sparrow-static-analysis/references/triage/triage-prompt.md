...
읽고, 이 항목이 **진성(true positive) / 위양성(false positive) / 보류** 중 무엇인지 판정하라.

규칙:

1. **판정 근거는 반드시 [체커 가이드]의 `## 진성 판별 기준` 또는 `## 흔한 위양성 패턴`에서 인용**하라.
...
5. **위양성**이면 `false_positive_reason`에 가이드의 어느 위양성 패턴에 해당하는지 + 어느 라인에서
   non-null/소유권 이전 등이 확정되는지 근거를 적어라. `fix`는 비운다.
